import { getEdgeCenter, Position, XYPosition } from "@xyflow/react";

export type HandlePosition = Position | 'center';
...
const handleDirections: Record<HandlePosition, XYPosition> = {
...
function getPoints({
  ...
  const targetDir = targetPosition === 'center'
    ? getDirection({ source: target, sourcePosition: 'center', target: source })
    : handleDirections[targetPosition];
 ...
 */
export function getSmoothStepPath2({
  sourceX,
  sourceY,
  sourcePosition = Position.Bottom,
  targetX,
